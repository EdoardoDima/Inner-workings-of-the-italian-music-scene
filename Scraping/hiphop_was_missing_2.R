...
###############  THIS IS EXACTLY THE SAME SCRAPING AS BEFORE, BUT      ############### 
###############  THE FIRST TIME I FORGOT TO SCRAPE THE HIP-HOP ARTISTS ############### 
###############  SO I DID IT HERE                                      ############### 
...
genres_spot = c('hip-hop', 'reggaeton')
...
for (genre in genres_spot){
  ...
  for (i in offsets){
    ...
    for (j in b$id){
      ...
      ita_term = c('ita', 'nap', 'neap', 'mila', 'roma')
...
write.csv(art4, "C:/Users/CreCre/Documents/artist_hiphop.csv", row.names=FALSE)'''
...
collab = data.frame(matrix(nrow = 1003, ncol = 400))
